...
public class cartDao {
	...
	public static List<cartDetail> DsCartDetail(int id) {
		...
		try {
			...
			ResultSet rs =state.executeQuery(query);
			while(rs.next()) {
				list.add(new cartDetail(new cart(rs.getInt(1), rs.getInt(2), rs.getInt(3), rs.getInt(4), rs.getString(5)), new shirt(rs.getInt(6), rs.getString(7), rs.getString(8), rs.getString(9), rs.getInt(10), rs.getString(10))));
			}
			db.close();
			return list;
		} catch (Exception e) {
